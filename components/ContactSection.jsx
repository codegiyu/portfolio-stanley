...
const ContactSection = () => {
    ...
    const handleFormSubmit = (e) =>{
        e.preventDefault();
        console.log(formValues, emailRef.current?.value);
    }
    return (
        <div className="relative flex flex-col bg-transparent px-4 py-8 lg:py-24 sm:px-8 md:px-16 lg:px-[2.5rem] font-montserrat">
           ...
           {/* Form Container */}
            <form onSubmit={handleFormSubmit} className="flex flex-col space-y-6 py-2 md:px-16 md:py-10 items-start md:flex-row md:justify-between md:items-center md:space-x-24 md:space-y-0 w-full">
                {/* Message Conatiner */}
                ...
                <div className="flex flex-col space-y-6 items-center w-[100%] md:w-[50%] px-2 py-8 md:py-20">
                    <input type="text" onChange={handleInputChange} id="name" name="name" value={formValues.name} className="w-[100%] px-10 py-4 rounded-xl focus:outline-none bg-grey" placeholder="Your Name" required/>
                    <input type="email" onChange={handleInputChange} value={formValues.email} id="email" name="email" className="w-[100%] px-10 py-4 rounded-xl focus:outline-none bg-grey" placeholder="Your Email Address" required/>
                    <textarea onChange={handleInputChange} className="bg-grey w-[100%] px-10 py-4 focus:outline-none rounded-xl" name="message" rows={5} id="message" placeholder="Your Message" required value={formValues.message}
                    >
                    </textarea>
                    {/* Submit Button */}
